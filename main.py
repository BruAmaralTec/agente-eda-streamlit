...
# Usamos o sessionStorage do navegador para controlar se a página já foi recarregada.
components.html(
    """
    <script>
...
st.set_page_config(page_title="Agente de E.D.A. Amaral", layout="wide")
...
# Garante que as variáveis da sessão existam desde o início.
...
# Este bloco só roda uma vez, quando um novo arquivo é carregado.
if uploaded_file is not None and st.session_state.df is None:
    try:
        # Limpa o sessionStorage para permitir o reload na próxima vez que um novo arquivo for carregado
        ...
        # Cria a ferramenta REPL com o dataframe carregado
        python_tool = create_python_analyst_tool(df)
        
        # Lista completa de todas as ferramentas disponíveis para o agente
        st.session_state.tools = [
            python_tool,
            plot_histogram,
            find_clusters,
